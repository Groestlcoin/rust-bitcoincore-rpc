...
use groestlcoin::secp256k1::Signature;
use groestlcoin::{
...
pub trait RpcApi: Sized {
    ...
    fn get_block_info(&self, hash: &groestlcoin::BlockHash) -> Result<json::GetBlockResult> {
    ...
    fn get_best_block_hash(&self) -> Result<groestlcoin::BlockHash> {
        self.call("getbestblockhash", &[])
    }

    /// Get block hash at a given height
    fn get_block_hash(&self, height: u64) -> Result<groestlcoin::BlockHash> {
    ...
    fn get_raw_transaction(
        ...
        block_hash: Option<&groestlcoin::BlockHash>,
    ) -> Result<Transaction> {
    ...
    fn get_block_filter(
        &self,
        block_hash: &groestlcoin::BlockHash,
    ...
    fn list_since_block(
        &self,
        blockhash: Option<&groestlcoin::BlockHash>,
    ...
    fn generate(&self, block_num: u64, maxtries: Option<u64>) -> Result<Vec<groestlcoin::BlockHash>> {
        self.call("generate", &[block_num.into(), opt_into_json(maxtries)?])
    }

    /// Mark a block as invalid by `block_hash`
    fn invalidate_block(&self, block_hash: &groestlcoin::BlockHash) -> Result<()> {
        self.call("invalidateblock", &[into_json(block_hash)?])
    }

    /// Mark a block as valid by `block_hash`
    fn reconsider_block(&self, block_hash: &groestlcoin::BlockHash) -> Result<()> {
        self.call("reconsiderblock", &[into_json(block_hash)?])
    }

    /// Get txids of all transactions in a memory pool
    fn get_raw_mempool(&self) -> Result<Vec<groestlcoin::Txid>> {
    ...
    fn send_raw_transaction<R: RawTx>(&self, tx: R) -> Result<groestlcoin::Txid> {
    ...
    fn wallet_process_psbt(
        ...
        let defaults = [
            true.into(),
            into_json(json::SigHashType::from(groestlcoin::SigHashType::All))?,
            true.into(),
        ];
...
impl fmt::Debug for Client {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "groestlcoincore_rpc::Client({:?})", self.client)
    }
}

impl Client {
    /// Creates a client to a groestlcoind JSON-RPC server.
    ///
    ...
    fn call<T: for<'a> serde::de::Deserialize<'a>>(
        ...
        let raw_args: Vec<_> = args
            ...
            .map(|a| {
                ...
                serde_json::value::RawValue::from_string(json_string) // we can't use to_raw_value here due to compat with Rust 1.41
            })
            .map(|a| a.map_err(|e| Error::Json(e)))
            .collect::<Result<Vec<_>>>()?;
        let req = self.client.build_request(&cmd, &raw_args);
        if log_enabled!(Debug) {
            debug!(target: "groestlcoincore_rpc", "JSON-RPC request: {} {}", cmd, serde_json::Value::from(args));
        }
...
fn log_response(cmd: &str, resp: &Result<jsonrpc::Response>) {
    if log_enabled!(Warn) || log_enabled!(Debug) || log_enabled!(Trace) {
        match resp {
            Err(ref e) => {
                if log_enabled!(Debug) {
                    debug!(target: "groestlcoincore_rpc", "JSON-RPC failed parsing reply of {}: {:?}", cmd, e);
                }
            }
            Ok(ref resp) => {
                if let Some(ref e) = resp.error {
                    if log_enabled!(Debug) {
                        debug!(target: "groestlcoincore_rpc", "JSON-RPC error for {}: {:?}", cmd, e);
                    }
                } else if log_enabled!(Trace) {
                    // we can't use to_raw_value here due to compat with Rust 1.41
                    let def = serde_json::value::RawValue::from_string(
                        serde_json::Value::Null.to_string(),
                    )
                    .unwrap();
                    let result = resp.result.as_ref().unwrap_or(&def);
                    trace!(target: "groestlcoincore_rpc", "JSON-RPC response for {}: {}", cmd, result);
                }
            }
        }
    }
